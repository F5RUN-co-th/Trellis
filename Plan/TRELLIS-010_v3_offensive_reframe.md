...
> **[traceability 07-09] เอกสารนี้ = DOCTRINE/MECHANISM (frozen).** ผล/เลข current-frame ทั้งหมด → **live SoR = `TRELLIS-010_LEDGER.md`** (keyed by Claim-ID) · เลขใน §9 ข้างล่าง = frozen historical snapshot · แก้ผล/สถานะที่ LEDGER ไม่ใช่ที่นี่ (กัน result เด้งไปมา · problem #3)
...
> ⬇ **[07-09] §9:89–145 PARTITION (accumulate — เก็บ reasoning/lineage ครบ · ไม่ strip):**
> · **frozen result-metric + forward-plan** (รวม experiment-order/REVISED-PLAN 144–145) → **สถานะ run-vs-open สด = LEDGER `## CURRENT FRONTIER`** (ห้ามอ่านว่า "executed/next" ที่นี่ — ชี้ LEDGER · crosswalk: fade=CLAIM-0003 · Discovery=CLAIM-0004 · v0=CLAIM-0005 · v1=CLAIM-0006 · gates=CLAIM-0007 · spread=CLAIM-0008 · TestB=CLAIM-0009 · real-exit=CLAIM-0010 · opportunity-unit=CLAIM-0002)
> · **decision-methodology = ยังใช้ (in-force doctrine · ไม่ frozen):** Gate-B thresholds (136) · decision-table (137–142) · MTC/capacity-probe (143) · Claim-Object (131) · evidence-promotion (129) · 4-leg gate
...
- **HYPOTHESIS ELIMINATION MATRIX (ChatGPT · full causal-chain — Sampling→Label→Representation→Optimization→Population→Execution→Deployment):** **⚠ [07-09] live open/eliminated = LEDGER `## CURRENT FRONTIER` (LEDGER:11 อ้างกลับมาบรรทัดนี้) · list ข้างล่าง = SNAPSHOT ณ 07-08 (ก่อน real-exit · ขาด row CLAIM-0010 "19-feat OHLC linear+GBM at real-exit = eliminated" โดยตั้งใจ — ไม่ patch กัน result เด้ง problem#3):** eliminated=[Representation: linear-univariate OHLCV · Optimization: linear E[signed-R] fit] · **ยังเปิด**=[Representation: nonlinear/event-stream/multi-scale · Label: exit-policy อื่น (1R/1.5R conditional) · Population: opportunity-conditioned/high-τ · Optimization: adaptive/rolling (concept-drift remedy) · no-directional-info-at-all] · **static-model = prior ลด ไม่ eliminate**
...
## 10. Governance
verify 3 ชั้น · expectancy-before-code · anti-curve-fit (numeric pre-register CONFIRM) · anti-flip-flop (เก็บ v4-entry) · **no-excuse** (ห้ามเปลี่ยนทุน/บัญชี/param/skip-ไม่มีเหตุผล/Stage-F-ก่อนเวลา) · ทุก decision root-cause-อธิบายได้-หรือ-reject · lockbox 2024–26 + guard 2021–23 (canonical = `brain_research_workflow.md §0`)
