...
- **⭐⭐ คอขวดจริง = DIRECTION NON-STATIONARITY (Engineer insight · Claude verified):** mean(rl−rs) **SIGN-FLIP** train −0.125 (short-better 12-16) → OOS +0.018 (long-better 19-20) · per-year แกว่ง −0.25..+0.12 → **payoff-direction ไม่ stationary ข้าม regime** · richer features บน target ที่ sign-flip = ล้มเหลว OOS เหมือนกัน
- **linear OHLCV = ไม่มี OOS cost-clearing directional edge** (สอดคล้อง info-theory ceiling) · **แต่ยังห้ามสรุป "ทำนายทิศไม่ได้"** (v0 = 1 family linear · §9 ต้องหลาย family ก่อน Stage-F)
- **v1 (NEXT · Engineer sharpening):** **sign-stability gate ก่อน** (feature-family ต้องโชว์ conditional-direction sign-stable train→OOS/per-regime — ไม่ผ่าน = dead ไม่ว่า expressive แค่ไหน) → แล้วเพิ่ม 1-2 family (nonlinear/GBM · event-stream representation) · info-theory-bounded · **ไม่ Stage-F**
