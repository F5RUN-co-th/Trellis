...
- **▶ Step 2 · Behavior→Discovery→Direction-Prediction:**
  ...
  - **★ 07-08 · in-hand direction-channel exhaustion + Test-B design PASS-but-low-value:**
    ...
    - **research ต่อ (ยังไม่จบ):** Test B **เสร็จ → direction load-bearing** (คำถามหลักตอบแล้ว) · **next #1 = re-measure DIRECTION PREDICTION ที่ exit จริง** (gates เดิม label ผิด): learned features ทำนายทิศชนะ v4 naive-breakout-rule ได้ไหม ที่ walk-exit → capture oracle-gap +7113 เพิ่ม (in-hand ก่อน) · #2 = tick-price channel · **แล้วค่อยตัดสิน Stage-F** · forward-test v4 = parallel validation (ไม่ใช่ endpoint)
