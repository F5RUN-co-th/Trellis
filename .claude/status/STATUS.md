...
- **▶ Step 2 · Behavior→Discovery→Direction-Prediction:**
  ...
  - **★ 07-08 · in-hand direction-channel exhaustion + Test-B design PASS-but-low-value:**
    ...
    - **★ 07-08 · DIRECTION-at-real-exit (label ถูก · leak-free)** (`Scripts/direction_at_real_exit.py`+`dir_features.py` · guards ผ่าน: mirror-sym + future-mask no-lookahead + reproduce mism==0 · R-norm payoff · per-fold train-only sign-gate · WF+embargo):
      ...
      - **finding หลัก: OHLC features near-totally NOT sign-stable** (kept เฉลี่ย **0.8/19** ต่อ fold · [0,1,2,1,1,0]) → learned direction ไม่ transfer
      - **learned (linear + GBM-fair) ไม่ชนะ baseline OOS:** linear lift **−0.31 CI[−0.663,+0.034] คร่อม 0** · GBM(sign-classifier weighted) **−0.28 CI[−0.735,+0.168] คร่อม 0** · active-fold −0.47 · **features มี weak-signal** (predictor −0.31 > perm-null −0.52 = +0.21) แต่ไม่พอชนะ baseline · MDE 0.35$/ไม้
      - **⚠ แก้ OVER-CLAIM 2 รอบ (Engineer จับ · Claude verified):** commit `a26a93e` "lift −0.52 CI<0 DECISIVE · harmful" = `fallback-to-all-19` artifact → แก้ abstain (CI คร่อม 0) · commit `b42278f` ยัง over-reach "exhausted → tick-price" + perm-null (จริงๆ = floor−baseline uninformative) + GBM stump → แก้: weak-signal test + active-fold + GBM capacity-fair + restate · **= over-claim pattern ของ Claude ซ้ำ 2 รอบ (จับ+แก้ครบ)**
      - **สรุป calibrated:** **v4 breakout-direction = ดี + ใกล้ OHLC-extractable ceiling** · learned OHLC ไม่เพิ่ม edge เหนือ MDE (**ไม่ใช่ "exhausted/proven-zero"** · sub-MDE ไม่ถูกตัด) · field SEARCH 2012-2020
    - **next (Engineer reframe · ไม่ escape):** **monetize direction ที่มี · อย่าไล่ล่า additive OHLC-direction** · **magnitude/opportunity channel (+1.745/+0.66) = positive อยู่แล้ว น่าพิจารณาก่อน** · tick-price = *hypothesis* ไม่ใช่ proven-need · forward-test v4 parallel
