...
- **▶ Step 2 · Behavior→Discovery→Direction-Prediction:**
  ...
  - **★ 07-08 · in-hand direction-channel exhaustion + Test-B design PASS-but-low-value:**
    ...
    - **★ 07-08 · DIRECTION-at-real-exit (label ถูก · leak-free) เสร็จ** (`Scripts/direction_at_real_exit.py`+`dir_features.py` · Engineer PASS หลายรอบ · guards ผ่าน: mirror-sym + future-mask no-lookahead + per-trade reproduce mism==0 · R-norm payoff · per-fold train-only sign-gate · expanding-WF+embargo):
      - **baseline (v4 breakout-dir rule) มี skill จริง** ที่ exit จริง (+0.385/ไม้ > floor coin-flip −0.198)
      - **learned OHLC features ไม่ชนะ baseline OOS** — predictor −0.14/ไม้ · **lift −0.52 CI[−0.95,−0.05] < 0** · ≈ **permutation-null −0.55** · flip-precision 45.3% < base-rate 47.6% → features **non-stationary · invert OOS** (คอขวด direction-non-stationarity ที่ documented ยืนอีกครั้ง)
      - **สรุปถูก (ไม่ใช่ "direction dead"):** v4 breakout-rule = direction-signal เดียวที่ใช้ได้จาก OHLC · **ไม่มี additive OHLC-feature direction signal OOS เกิน breakout-rule** · oracle +7113 = OHLC จับไม่ได้ · field SEARCH 2012-2020 · MDE 0.45$/ไม้
    - **next:** #1 = tick-price channel (in-hand · DPI headroom เหนือ OHLC) · #2 = Stage-F (ถ้า tick ก็ตาย) · forward-test v4 = parallel validation
