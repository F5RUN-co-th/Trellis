...
- **▶ Step 2 · Behavior→Discovery→Direction-Prediction:**
  ...
  - **★ 07-08 · in-hand direction-channel exhaustion + Test-B design PASS-but-low-value:**
    - **⚠ [แก้ over-claim 07-08] การประกาศเดิม "direction unpredictable ทุก channel" = ผิด** — ทุก gate (MI-ceiling/Gate C/gate2/magnitude/spread/tickvol) วัดใต้ label `trade_R` **1R/1.5R-intraday เท่านั้น** · ที่ถูก: **LEARNED features ไม่เพิ่ม OOS direction skill ใต้ label นั้น** (OHLC sign-MI 1/19 · tickvol 0/7 · spread dead de-cluster[0,0,0]/OOS+0.004R) · **Test B พิสูจน์ direction มี skill ที่ exit จริง → "direction ตาย" = artifact ของ label ผิด · gates ต้อง re-measure ที่ exit จริง** · magnitude predictable +0.66 (label-agnostic)
    ...
    - **research ต่อ (ยังไม่จบ):** Test B **เสร็จ → direction load-bearing** (คำถามหลักตอบแล้ว)
    - **★ 07-08 · DIRECTION-at-real-exit (label ถูก · leak-free)** (`Scripts/direction_at_real_exit.py`+`dir_features.py` · guards ผ่าน: mirror-sym + future-mask no-lookahead + reproduce mism==0 · R-norm payoff · per-fold train-only sign-gate · WF+embargo):
      - **baseline (v4 breakout-dir rule) มี skill จริง** ที่ exit จริง (+0.385/ไม้ > floor coin-flip −0.198)
      - **[SEED-ROBUST · 9-root multi-seed pass ใน script] v4 breakout-direction = edge จริง:** base−floor +0.582$/ไม้ · **lower-bound > 0 ทุก 9 seed** (min +0.133) = distinguishable จาก coin-flip
      - **[SEED-ROBUST] learned OHLC (19-feat · linear+GBM-fair) ไม่มี additive edge:** aggregate **< 0 ทุก seed** (−0.39..−0.02) · **ไม่มี seed ไหน CI-excl-0-positive** · point-negative ทุก estimator = inconclusive-to-null (ไม่ใช่ harmful)
      - **gate descriptors (abstain 17–66% · kept ~0–12/19) = SEED-FRAGILE → ไม่ freeze ค่าเดียว**
      - **⚠ OVER-CLAIM 3 รอบ + rng-defect + single-draw-descriptor (Engineer จับ 6 รอบ · Claude verified ทุกครั้ง):** `a26a93e` fallback-artifact · `b42278f` perm-null+stump · `9e846b4` N=1 weak-signal+circular-ceiling · `8f6b8e6` baseline ไม่มี CI · `[rng-fix]` shared-rng order-dependent → per-call-site crc32-seeded → deterministic · `[multi-seed]` frozen descriptor = 1 draw ที่ extreme → multi-seed invariants · **meta-pattern (Engineer diagnose): แต่ละ fix ลบ affirmative เก่า→ใส่นุ่มกว่าแทน เพื่อหลบผลลบแบนๆ**
      - **TERMINAL (seed-robust · deterministic):** **19 OHLC features ไม่มีหลักฐาน additive direction signal ที่ real exit** (seed-robust · linear+GBM) · **v4 breakout-direction = edge จริง CI-backed** · **ceiling ยังไม่วัด · oracle +4.98 → headroom +4.6 เหลือ = direction ยังไม่ solved** · **ไม่ใช่ "proven-zero"** · SEARCH 2012-2020
    - **next: ไม่ prescribe (ให้ Win ตัดสิน)** — options: richer OHLC representation · tick-price (hypothesis) · magnitude channel (+1.745 = n=31 unvalidated · +0.66 sturdier) · forward-test v4 · monetize v4 direction ที่ดีอยู่
