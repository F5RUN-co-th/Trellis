...
- **▶ Step 2 · Behavior→Discovery→Direction-Prediction:**
  ...
  - **★ 07-08 · in-hand direction-channel exhaustion + Test-B design PASS-but-low-value:**
    ...
    - **★ 07-08 · DIRECTION-at-real-exit (label ถูก · leak-free)** (`Scripts/direction_at_real_exit.py`+`dir_features.py` · guards ผ่าน: mirror-sym + future-mask no-lookahead + reproduce mism==0 · R-norm payoff · per-fold train-only sign-gate · WF+embargo):
      ...
      - **ไม่พบ additive OHLC-feature direction edge OOS — ทั้ง signed-R LINEAR และ GBM nonlinear:** linear lift **−0.31 CI[−0.663,+0.034] คร่อม 0** · GBM **−0.11 CI[−0.587,+0.399] คร่อม 0** · predictor ≈ perm-null · **learner degenerate = constant-dir bet** (predLongFrac 0.0 หลาย fold · direction non-stationary) · MDE 0.35$/ไม้
      - **⚠ แก้ OVER-CLAIM (Engineer จับ · Claude verified · commit แรก `a26a93e` ผิด):** เดิมเขียน "lift −0.52 CI[−0.95,−0.05]<0 DECISIVE · features harmful" = **artifact ของ `fallback-to-all-19`** (2 fold zero-stable → overfit) · แก้เป็น **abstain-to-baseline** → CI **คร่อม 0** · verdict key ที่ **perm-null ไม่ใช่ baseline** · = pattern over-claim ของ Claude ซ้ำ (จับ+แก้แล้ว)
      - **สรุปถูก:** v4 breakout-rule = direction-signal เดียวที่ใช้ได้จาก OHLC · **ไม่มี additive OHLC-feature direction edge OOS (linear+GBM 2 model class · MDE-bounded)** — ไม่ใช่ "proven zero" แต่ standard models จับไม่ได้เหนือ MDE · field SEARCH 2012-2020
    - **next:** tick-price channel (in-hand · DPI headroom เหนือ OHLC) → Stage-F ถ้าตาย · forward-test v4 = parallel validation
