...
- **▶ Step 2 · Behavior→Discovery→Direction-Prediction:**
  ...
  - **★ 07-08 · in-hand direction-channel exhaustion + Test-B design PASS-but-low-value:**
    ...
    - **★ 07-08 · DIRECTION-at-real-exit (label ถูก · leak-free)** (`Scripts/direction_at_real_exit.py`+`dir_features.py` · guards ผ่าน: mirror-sym + future-mask no-lookahead + reproduce mism==0 · R-norm payoff · per-fold train-only sign-gate · WF+embargo):
      ...
      - **baseline (v4 breakout-rule) มี direction skill +0.385$/ไม้** (> floor −0.198) = fact ของ rule ที่มีอยู่
      - **learned OHLC (19-feat · linear+GBM-fair) ไม่ชนะ baseline · point-negative ทุก estimator:** aggregate −0.31 CI[−0.663,+0.034] คร่อม 0 (เจือจาง abstain 34%) · **ACTIVE-fold −0.47 CI[−0.971,−0.010]** (hurt เมื่อ gate ใช้ feature · marginally-sig) · GBM −0.28 · **OHLC features ~95% NOT sign-stable** (kept 0.8/19)
      - **⚠ OVER-CLAIM 3 รอบ (Engineer จับ · Claude verified ทุกรอบ):** `a26a93e` "CI<0 harmful" = fallback artifact · `b42278f` "exhausted→tick-price" = perm-null(=floor−baseline)+stump-GBM · `9e846b4` "weak-signal +0.21" = **N=1 shuffle ไม่มี null-band** + "near ceiling" circular · **meta-pattern (Engineer diagnose): แต่ละ fix ลบ affirmative เก่า→ใส่ affirmative นุ่มกว่าแทน เพื่อหลบผลลบแบนๆ** — จับ+แก้ครบ
      - **TERMINAL (flat · ไม่ manufacture positive):** **19 OHLC features เหล่านี้ไม่มี usable additive direction signal ที่ real exit** (point-negative เมื่อ act) · **OHLC extractable ceiling = ยังไม่วัด · oracle +4.98 → headroom +4.6$/ไม้ ยังเหลือ = direction ยังไม่ solved** (แค่ 19-feat นี้จับไม่ได้) · **ไม่ใช่ "proven-zero"** · SEARCH 2012-2020
